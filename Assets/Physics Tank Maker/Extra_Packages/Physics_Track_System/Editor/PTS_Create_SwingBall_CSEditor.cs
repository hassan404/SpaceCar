...
namespace ChobiAssets.PTS
{
	...
	[ CustomEditor (typeof(PTS_Create_SwingBall_CS))]
	public class PTS_Create_SwingBall_CSEditor : Editor
	{
		...
		void Create_Ball (string direction, Vector3 position, int number)
		{
            ...
            {
                case 0:
                    gameObject.layer = PTS_Layer_Settings_CS.Reinforce_Layer; // Reinforce (ignore all collisions)
        ...
        void Find_RoadWheels()
        {
            ...
            PTS_Drive_Wheel_CS[] driveScripts = thisTransform.parent.GetComponentsInChildren<PTS_Drive_Wheel_CS>();
            foreach (PTS_Drive_Wheel_CS driveScript in driveScripts)
            {
                Transform wheelTransform = driveScript.transform;
                if (wheelTransform.parent.GetComponent<PTS_Create_RoadWheel_CS>() && wheelTransform.GetComponent<MeshFilter>())
                { // RoadWheel && visible
        ...
        void Find_SupportWheels()
        {
            ...
            List<Vector3> posList = new List<Vector3>();
            PTS_Static_Wheel_CS[] wheelScripts = thisTransform.parent.GetComponentsInChildren<PTS_Static_Wheel_CS>();
            foreach (PTS_Static_Wheel_CS wheelScript in wheelScripts)
            {
                Transform wheelTransform = wheelScript.transform;
                if (wheelTransform.parent.GetComponent<PTS_Create_SupportWheel_CS>())
                { // SupportWheel
                    if (wheelTransform.localPosition.y > 0.0f)
                    { // Left
                        Vector3 wheelPos = wheelTransform.position;
